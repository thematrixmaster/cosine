...
def parse_args():
    ...
    # Sampling Params
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Number of independent trajectories"
    )
    parser.add_argument(
        "--max-mutations", type=int, default=10, help="Maximum mutations allowed per trajectory"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Sampling temperature for energy distribution",
    )
    parser.add_argument(
        "--min-improvement", type=float, default=1e-4, help="Stop if gain is below this threshold"
    )
    ...
    parser.add_argument(
        "--oracle-chunk-size", type=int, default=2000, help="Batch size for brute-force inference"
    )
...
def run_hotflip_trajectory(
    start_seq: str, oracle, args, mask: np.ndarray, device
) -> tuple[str, int, float]:
    ...
    for step in range(args.max_mutations):
        ...
        if args.use_gradients:
            candidates, gains, scores = get_gains_gradients(
                current_seq, current_score, mask, oracle, args
            )
        else:
            candidates, gains, scores = get_gains_brute_force(
                current_seq, current_score, mask, oracle, args
            )
...
def main():
    ...
    for i in tqdm(range(args.batch_size)):
        ...
        final_seq, n_muts, final_score = run_hotflip_trajectory(
            start_seq=seed_seq, oracle=oracle, args=args, mask=mask, device=device
        )
    ...
    if args.output_path is None:
        ...
        args.output_path = (
            f"hotflip_{method_str}_{args.oracle}_seed{args.oracle_seed_idx}_k{args.top_k}_{ts}.csv"
        )
