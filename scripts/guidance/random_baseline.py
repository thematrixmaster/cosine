...
def parse_args():
    ...
    # Sampling Params
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Number of random sequences to generate"
    )
    parser.add_argument(
        "--max-mutations", type=int, default=5, help="Number of random mutations to apply"
    )
...
def apply_random_mutations(seed_seq: str, n_mutations: int, mask: np.ndarray) -> str:
    """
...
def main():
    ...
    for i in tqdm(range(args.batch_size)):
        ...
        mutant_seq = apply_random_mutations(
            seed_seq=seed_seq, n_mutations=args.max_mutations, mask=mask
        )
    ...
    if args.output_path is None:
        ...
        args.output_path = (
            f"random_{args.oracle}_seed{args.oracle_seed_idx}_mut{args.max_mutations}_{ts}.csv"
        )
