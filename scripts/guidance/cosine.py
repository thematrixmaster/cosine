...
def load_oracle_binders(oracle_variant: str) -> pd.DataFrame:
    ...
    # Get CSV path
    module_dir = Path("evo/evo/oracles/data")
...
def sample_single_trajectory(
    ...
    for retry in range(max_retries):
        ...
        t_rep = (
            torch.tensor([branch_length] * n_sequences, device=device)
        ...
        x_sizes = torch.nn.functional.pad(x_sizes, (0, len(x_rep[0]) - len(x_sizes)), value=0).view(
            1, -1
        )
...
def sample_with_mutation_ceiling(
    ...
    print(error_msg)
    raise RuntimeError(
        f"Mutation ceiling of {max_mutations} not achievable after {max_retries} attempts"
    )
...
def main():
    ...
    module = CTMCModule.load_from_checkpoint(
        str(args.model_path), map_location=device, strict=False
    )
    ...
    if args.seed_seq is not None:
        ...
        seed_seq = select_oracle_seed(oracle_variant=args.oracle, seed_idx=args.oracle_seed_idx)
    ...
    if args.mask_region is not None:
        ...
        padded_mask = np.pad(
            mask_array, (int(vocab.prepend_bos), int(vocab.append_eos)), constant_values=False
        )
    ...
    if args.use_discrete_steps:
        ...
        assert (
            args.max_mutations is not None
        ), "When using discrete steps, --max-mutations must be specified"
    ...
    print(
        f"Region mask: {args.mask_region if args.mask_region else 'None (all positions allowed)'}"
    )
    ...
    def unguided_generate_fn(
        x: Tensor, t: Tensor, device, p, max_decode_steps, x_sizes, mask=None, **kwargs
    ):
    ...
    for method in methods:
        ...
        print(
            f"Avg OASIS Humanness: {method_df['oasis_humanness'].mean():.4f} +/- {method_df['oasis_humanness'].std():.4f}"
        )
        print(
            f"Avg IGLM Humanness: {method_df['iglm_humanness'].mean():.4f} +/- {method_df['iglm_humanness'].std():.4f}"
        )
        ...
        print(
            f"Avg Mutations: {method_df['n_mutations'].mean():.4f} +/- {method_df['n_mutations'].std():.4f}"
        )
