"""Timing experiment sweeping over sequence length.

This script measures the runtime of guided vs unguided sampling across different
sequence lengths: [50, 100, 250, 500] with a fixed 50 Gillespie steps.

Usage:
    uv run python scripts/guidance/timing_experiment_seqlen.py \
        --oracle SARSCoV2Beta \
        --seed 42
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Timing experiment: sweep over sequence length'
    )
    ...
    parser.add_argument('--fixed-steps', type=int, default=50,
                        help='Fixed number of Gillespie steps (default: 50)')
...
def main():
    ...
    seq_lengths = [50, 100, 250, 500]
    fixed_steps = args.fixed_steps
    timing_results = []

    print("=" * 80)
    print("RUNNING TIMING EXPERIMENT - SEQUENCE LENGTH SWEEP")
    print("=" * 80)
    print(f"Fixed steps: {fixed_steps}")
    print(f"Sequence lengths: {seq_lengths}")
    print(f"Random seed: {random_seed}\n")

    # Warmup run
    warmup_seq = generate_random_protein_sequence(100, seed=random_seed)
    print(f"Running warmup (length=100, steps={fixed_steps})...")
    warmup_start = time.perf_counter()
    _ = sample_with_mutation_ceiling(
        generate_fn=guided_generate_fn,
        seed_seq=warmup_seq,
        branch_length=float(fixed_steps),
        vocab=vocab,
        device=device,
    ...
    for method_name, generate_fn in [('Unguided', unguided_generate_fn), ('Guided', guided_generate_fn)]:
        print(f"\n{'='*80}")
        print(f"{method_name} Sampling (Fixed {fixed_steps} steps)")
        print(f"{'='*80}")
        ...
        for seq_len in seq_lengths:
            ...
            current_seed_seq = generate_random_protein_sequence(seq_len, seed=random_seed)
    ...
    print("\n" + "=" * 80)
    print(f"SEQUENCE LENGTH TIMING RESULTS (Guided vs Unguided, {fixed_steps} steps)")
    print("=" * 80)

    # Organize results by sequence length
    results_by_len = {}
    for result in timing_results:
        length = result['seq_length']
        if length not in results_by_len:
            results_by_len[length] = {}
        results_by_len[length][result['method']] = result

    # Print header
    print(f"{'Seq Len':<10} | {'Unguided (s)':<14} {'Guided (s)':<14} {'Speedup':<10} | {'Fitness (U)':<12} {'Fitness (G)':<12} | {'Mutations':<10}")
    print("-" * 110)

    # Print rows
    for length in sorted(results_by_len.keys()):
        unguided = results_by_len[length].get('Unguided', {})
        guided = results_by_len[length].get('Guided', {})
        ...
        print(f"{length:<10} | {u_time:<14.4f} {g_time:<14.4f} {speedup:<10.2f} | {u_fitness:<12.4f} {g_fitness:<12.4f} | {mutations:<10}")
