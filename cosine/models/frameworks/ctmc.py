...
def simulate_evolution_with_rejection_sampling(
    ...
    **kwargs,
    ...
    """
    Simulates evolution on a fixed tree with given branch lengths using a trained model.
...
def simulate_evolution_with_rejection_sampling_batched(
    ...
    **kwargs,
    ...
    while bfs_queue:
        ...
        for child in node.children:
            node_info[child.name] = (
                node.name,
                child.dist,
                [c.name for c in child.children],
                child.is_leaf(),
            )
    ...
    for node_name, parent_name, branch_length in node_order:
            ...
            if chosen_sequence is None:
                if retry_count < max_retries:
                    if verbose and retry_count == 0:
                        print(
                            f"Realization {real_idx}, node {node_name}: Failed length filter, retrying..."
                        )
                    ...
                    x_retry = torch.from_numpy(vocab.encode_batched_sequences(retry_batch)).to(
                        device
                    )
                    t_retry = (
                        torch.tensor([branch_length] * n_sequences, dtype=torch.float32)
                        .unsqueeze(-1)
                        .to(device)
                    )
                    ...
                    chosen_sequence = filter_sequences(
                        retry_decoded, length_criterion, likelihood_fn
                    )
                ...
                if chosen_sequence is None:
                    if verbose:
                        print(
                            f"Realization {real_idx}, node {node_name}: Max retries reached, using fallback"
                        )
                    chosen_sequence = random.choice(node_sequences)
        ...
        # Progress update
        if verbose and (
            nodes_processed % (num_realizations * 5) == 0 or nodes_processed == total_nodes
        ):
    ...
    if verbose:
        ...
        if len(leaves) > 0:
            ...
            print(
                f"Independence check: Leaf '{example_leaf}' has {unique_seqs}/{num_realizations} unique sequences"
            )
...
def decode_sequence_from_toks(toks: torch.Tensor, vocab: Vocab):
    tokens = []
    toks_to_skip = [
        vocab.bos_idx,
        vocab.unk_idx,
...
def infer_log_likelihoods(dataloader: DataLoader, net: nn.Module, vocab: Vocab, n_batches=100):
